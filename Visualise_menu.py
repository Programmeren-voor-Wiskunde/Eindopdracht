...
while menu1 == True:
    ...
    menu1_text=font.render("Onder welke naam wilt u spelen?",True,(255,255,255))
    screen.blit(menu1_text,(input_rect.x,input_rect.y - 30))  
  
    pygame.draw.rect(screen,color,input_rect, 2)
    menu1_inputrect=font.render(menu1_input,True,(255,255,255))
    screen.blit(menu1_inputrect,(input_rect.x+5,input_rect.y + 5))
...
while menu2 == True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_BACKSPACE:
                ...
                game.difficulty=int(menu2_input)
                menu2=False
                break
            elif type(event.key) == int:
                menu2_input+=event.unicode
            
    #keeps menu1 with input on the screen        
    menu1_text=font.render("Onder welke naam wilt u spelen?",True,(255,255,255))
    screen.blit(menu1_text,(input_rect.x,input_rect.y - 30))  
    ...
    #graphic rendering of menu2
    menu2_text=font.render("Moeilijkheidsgraad: Aantal seconden voor computer een set vindt.",True,(255,255,255))
    screen.blit(menu2_text,(input_rect.x,input_rect.y + 70))  
  
    pygame.draw.rect(screen,color,(input_rect.x,input_rect.y+ 105, 240, 32), 2)
    menu2_inputrect=font.render(menu2_input,True,(255,255,255))
    screen.blit(menu2_inputrect,(input_rect.x+5,input_rect.y + 110))
...
# initialises the screen with not_a_set_message = False and overtime_message=False and the user_nam given in the menu
visualise_set_of_twelve(game.set_of_twelve, selected_cards, False, False, [0,0])
